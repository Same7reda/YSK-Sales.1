...
import { HomeIcon, PackageIcon, FileTextIcon, UsersIcon, BarChartIcon, SettingsIcon, ReceiptIcon, TruckIcon, WalletIcon, CalendarIcon, HistoryIcon, ClipboardListIcon, ClipboardCheckIcon, TicketIcon } from './components/icons';
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    if (currentUser) {
        const visiblePages = currentUser.permissions.visiblePages;
        // If the current active page is not in the user's visible pages list
        if (!visiblePages.includes(activePage)) {
            // Find the first page from the sidebar's order that the user is allowed to see
            const firstAllowedPage = navItemsList.find(item => visiblePages.includes(item.page));
            if (firstAllowedPage) {
                setActivePage(firstAllowedPage.page);
            }
        }
    }
...
const AppRouter: React.FC = () => {
    ...
    if (!isInitialized || (isLoading && activationStatus === 'pending')) {
